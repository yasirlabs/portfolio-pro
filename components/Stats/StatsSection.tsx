...
const AnimatedStatsBar: React.FC = () => {
  ...
  const years = useCounter(5, 2000);
  const projects = useCounter(54, 2500);
  const clients = useCounter(2, 2200);
  const awards = useCounter(48, 2300);
    ...
    >
      ...
      <div ref={statsRef} className="w-full max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          animate={!isMobile && isInView ? { opacity: 1, y: 0 } : isMobile ? {} : { opacity: 0, y: -30 }}
          transition={{ duration: 0.6 }}
        >
          <motion.div
            className="inline-block mb-4"
            initial={isMobile ? {} : { scale: 0 }}
            animate={!isMobile && isInView ? { scale: 1 } : isMobile ? {} : { scale: 0 }}
            ...
            return (
              ...
              >
                ...
                <div className="relative bg-gradient-to-br from-dark-secondary to-dark rounded-3xl p-8 sm:p-10 lg:p-12 glow-box transition-all duration-500 overflow-hidden border border-border-subtle group-hover:border-primary/50 min-h-[320px] sm:min-h-[360px] lg:min-h-[400px] flex flex-col justify-between">
                  {/* Particles */}
                  <Particles isHovered={hoveredIndex === index} isMobile={isMobile} />
                  ...
                  <div className="relative z-10">
                    ...
                    <motion.div
                      ...
                      animate={!isMobile && hoveredIndex === index ? { x: 5 } : { x: 0 }}
                      transition={{ duration: 0.3 }}
                    >
                      {stat.label}
                    </motion.div>
                    ...
                    <div className="relative flex items-baseline gap-2 mb-4">
                      <motion.span
                        ...
                        animate={
                          !isMobile && hoveredIndex === index ? { scale: 1.1 } : { scale: 1 }
                        }
                        transition={{
                          duration: 0.3,
                          type: "spring",
                    ...
                    <motion.div
                      className={`text-sm sm:text-base lg:text-lg font-medium tracking-wide transition-colors duration-300 ${
                        !isMobile && hoveredIndex === index ? "text-text/80" : "text-text/40"
                      }`}
                      animate={!isMobile && hoveredIndex === index ? { x: 5 } : { x: 0 }}
                      transition={{ duration: 0.3 }}
                    >
        ...
        >
          <div className="inline-flex items-center gap-2 text-sm text-text/40">
            <motion.div
              ...
              animate={isMobile || !isMounted ? {} : { scale: [1, 1.3, 1], opacity: [0.5, 1, 0.5] }}
